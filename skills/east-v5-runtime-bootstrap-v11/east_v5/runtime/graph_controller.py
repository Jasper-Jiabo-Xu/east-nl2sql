"""Fail-closed, data-free v11 full runtime graph controller."""
...
class GraphController:
    ...
    def __init__(self, skill_root: Path, runtime_root: Path):
        ...
        if self.graph.get("schema_version") != "east-v5-full-runtime-graph/v11" or self.manifest.get("skill_name") != "east-v5-runtime-bootstrap-v11":
    ...
    def _state_path(self) -> Path:
        return self.runtime_root / "east-v5-full-runtime-v11-state.json"
    ...
    def _root(self, binding: str) -> None:
        ...
        marker = self.runtime_root / "daemon-root-binding-v11.json"
        try:
        ...
        if _load(marker, "RUNTIME_ROOT_BINDING_INVALID") != {"schema_version": "east-v5-daemon-root-binding/v11", "root_binding_id": binding}:
            raise GraphError("RUNTIME_ROOT_BINDING_INVALID")

    def _state(self) -> dict[str, Any]:
        if not self._state_path.exists():
            return {"schema_version": "east-v5-full-runtime-state/v11", "preflights": {}, "runs": {}}
        value = _load(self._state_path, "RUNTIME_STATE_INVALID")
        if set(value) != {"schema_version", "preflights", "runs"} or value["schema_version"] != "east-v5-full-runtime-state/v11" or not isinstance(value["preflights"], dict) or not isinstance(value["runs"], dict):
    ...
    def _validate_envelope(self, envelope: dict[str, Any], claim: dict[str, Any], state: dict[str, Any]) -> None:
        ...
        if set(envelope) != needed or envelope.get("schema_version") != "runtime_graph_envelope/v11" or envelope.get("mode") not in {"event", "foundation"} or not isinstance(envelope.get("run_id"), str) or not envelope["run_id"] or not isinstance(envelope.get("attempt"), int) or envelope["attempt"] not in {1, 2, 3} or envelope.get("outcome") not in {"success", "failure"} or not isinstance(envelope.get("input_receipt_hashes"), list) or len(envelope["input_receipt_hashes"]) != len(set(envelope["input_receipt_hashes"])):
    ...
    def _new_envelope(self, prior: dict[str, Any], target: str, inputs: list[str], attempt: int | None = None) -> dict[str, Any]:
        return {"schema_version": "runtime_graph_envelope/v11", "run_id": prior["run_id"], "mode": prior["mode"], "attempt": prior["attempt"] if attempt is None else attempt, "target_agent_id": target, "target_agent_uuid": self.agents[target]["uuid"], "root_binding_id": prior["root_binding_id"], "preflight_token": prior["preflight_token"], "input_receipt_hashes": inputs, "outcome": "success"}
    ...
    def run_task(self, envelope: dict[str, Any], claim: dict[str, Any], task_id: str) -> dict[str, Any]:
        ...
        if envelope["outcome"] == "failure":
            ...
            receipt = {"schema_version": "east-v5-runtime-receipt/v11", "run_id": envelope["run_id"], "agent_id": target, "attempt": envelope["attempt"], "input_receipt_hashes": envelope["input_receipt_hashes"]}
