"""Self-contained v5 controller with cwd-confined launch descriptors."""
...
SKILL_NAME = "east-v5-runtime-bootstrap-v5"
SKILL_VERSION = "v5"
...
class RuntimeController:
    ...
    def _validate_manifest(self) -> None:
        ...
        if set(self.manifest) != needed or self.manifest.get("schema_version") != "workspace_skill_bundle_manifest/v5" or (self.manifest.get("skill_name"), self.manifest.get("skill_version")) != (SKILL_NAME, SKILL_VERSION):
    ...
    def _validate_envelope(self) -> None:
        ...
        if keys != required and keys != required | {"launch_idempotency_key"} or self.envelope.get("schema_version") != "task_input_envelope/v5" or self.envelope.get("adapter_version") != "east-v5-runtime-adapter/v5" or self.envelope.get("attempt") not in {1, 2, 3}:
        ...
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v5":
    ...
    def _validate_control_envelope(self) -> None:
        ...
        bootstrap = self.envelope["execution_bootstrap"]
        keys = {"bootstrap_version", "candidate_base_sha", "candidate_head_sha", "controller_sha256", "adapter_sha256", "runtime_context", "skill_bundle"}
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v5" or bootstrap.get("candidate_head_sha") != self.manifest["source_candidate_head"] or not _GIT.fullmatch(str(bootstrap.get("candidate_base_sha"))):
    ...
    def _outbox_state(self, path: Path) -> dict[str, Any]:
        state = _load(path, "RUNTIME_LAUNCH_CREATE_OUTPUT_INVALID") if path.exists() else {"schema_version": "EAST-V5-LAUNCH-OUTBOX/v5", "entries": {}}
        if set(state) != {"schema_version", "entries"} or state["schema_version"] != "EAST-V5-LAUNCH-OUTBOX/v5" or not isinstance(state["entries"], dict):
