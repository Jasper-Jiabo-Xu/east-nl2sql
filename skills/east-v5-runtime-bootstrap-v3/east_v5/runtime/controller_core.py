"""Self-contained v3 controller with recoverable outbox and staged edges."""
...
SKILL_NAME = "east-v5-runtime-bootstrap-v3"
SKILL_VERSION = "v3"
...
class RuntimeController:
    ...
    def _validate_manifest(self) -> None:
        ...
        if set(self.manifest) != needed or self.manifest.get("schema_version") != "workspace_skill_bundle_manifest/v3" or (self.manifest.get("skill_name"), self.manifest.get("skill_version")) != (SKILL_NAME, SKILL_VERSION):
    ...
    def _validate_envelope(self) -> None:
        required = {"schema_version", "adapter_version", "issue_id", "platform_parent_issue_id", "project_id", "run_id", "trace_id", "qa_id", "attempt", "target_agent_id", "target_agent_uuid", "root_binding_id", "input_ref", "input_receipt", "expected_output", "execution_bootstrap", "recovery_of"}
        keys = set(self.envelope)
        if keys != required and keys != required | {"launch_idempotency_key"} or self.envelope.get("schema_version") != "task_input_envelope/v3" or self.envelope.get("adapter_version") != "east-v5-runtime-adapter/v3" or self.envelope.get("attempt") not in {1, 2, 3}:
        ...
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v3":
        ...
        if not isinstance(recovery, dict) or set(recovery) != {"old_run_id", "old_issue_id", "old_task_id", "decision_comment_id"}:
            _fail("RUNTIME_RECOVERY_BINDING_INVALID")
        if self.envelope["attempt"] == 1 and recovery != {"old_run_id": "real-probe-010-110-120-v2", "old_issue_id": "0081927a-f1e7-42fe-a9e7-0a080857c082", "old_task_id": "c73f6821-0c29-4941-9214-12986df0d389", "decision_comment_id": "7397c95e-15b6-4b60-9d2d-5693d1e2659c"}:
            _fail("RUNTIME_RECOVERY_BINDING_INVALID")
    ...
    def _outbox_state(self, path: Path) -> dict[str, Any]:
        return _load(path, "RUNTIME_LAUNCH_CREATE_OUTPUT_INVALID") if path.exists() else {"schema_version": "EAST-V5-LAUNCH-OUTBOX/v3", "entries": {}}
    ...
    def _launch(self, registry: ArtifactRegistry, next_envelope: dict[str, Any], *, record: Path | None, fail: bool) -> dict[str, Any]:
        ...
        key = hashlib.sha256(canonical_bytes({"parent": self.envelope["platform_parent_issue_id"], "run": self.envelope["run_id"], "attempt": self.envelope["attempt"], "from": self.envelope["target_agent_id"], "to": target, "input": next_envelope["input_ref"]})).hexdigest()
        ...
        entry = outbox["entries"].setdefault(key, {"state": "intent", "target": target})
        ...
        if entry.get("issue_id") is None:
            descriptor = registry.directory / ("launch-" + key + ".json")
            descriptor.write_text(json.dumps(next_envelope, ensure_ascii=False, sort_keys=True), encoding="utf-8")
            command = ["multica", "issue", "create", "--title", f"EAS runtime {self.envelope['run_id']} {target} attempt {self.envelope['attempt']}", "--description-file", str(descriptor), "--parent", self.envelope["platform_parent_issue_id"], "--assignee-id", next_envelope["target_agent_uuid"], "--status", "todo", "--project", self.envelope["project_id"], "--allow-duplicate", "--output", "json"]
            try:
