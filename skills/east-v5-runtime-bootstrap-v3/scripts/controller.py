#!/usr/bin/env python3
"""Single executable entrypoint for the immutable v3 workspace Skill."""
    ...
    try:
        ...
        if args.command == "launcher-control-preflight":
            if not args.runtime_root or not args.task_id or not args.runtime_id:
                raise ControllerError("RUNTIME_RUN_ARGUMENT_MISSING")
            # The caller supplies --launcher-record only in isolated tests; production uses _launch.
            print(json.dumps({"status": "launcher_control_ready", "business_operation": "not_started"}, sort_keys=True))
            return 0
        if not args.runtime_root or not args.task_id or not args.runtime_id:
            raise ControllerError("RUNTIME_RUN_ARGUMENT_MISSING")
