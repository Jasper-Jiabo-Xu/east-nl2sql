#!/usr/bin/env python3
"""Build the immutable v8 installation archive from an already-committed head."""
...
_SOURCE_DESCRIPTION = b"description: Execute the EAST V5 v8 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task.\n"
_RUNTIME_DESCRIPTION = b'description: "Execute the EAST V5 v8 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task."\n'
...
def build(repo_root: Path, head: str, output: Path) -> dict[str, Any]:
    ...
    skill_prefix = "skills/east-v5-runtime-bootstrap-v8/"
    ...
    return {"schema_version": "workspace_skill_bundle_build_receipt/v8", "skill_name": manifest["skill_name"], "skill_version": manifest["skill_version"], "source_candidate_head": head, "archive_sha256": archive_hash, "manifest_sha256": manifest_hash, "supporting_files": manifest["files"], "skill_metadata": manifest["skill_metadata"], "skill_identity": skill_identity}
