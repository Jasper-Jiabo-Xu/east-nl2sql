"""Self-contained v8 controller with cwd-confined launch descriptors."""
...
from east_v5.governance import ContractError, canonical_bytes, file_sha256, resolve_runtime_root, root_binding_id, sha256

SKILL_NAME = "east-v5-runtime-bootstrap-v8"
SKILL_VERSION = "v8"
MATERIALIZER_VERSION = "multica_daemon_frontmatter_normalizer/v8"
...
class RuntimeController:
    ...
    def __init__(self, skill_root: Path, envelope: dict[str, Any], claim: dict[str, Any]):
        ...
        self.control_mode = envelope.get("execution_mode") == "launcher-control-preflight/v8"
        self._validate_manifest()
        (self._validate_control_envelope if self.control_mode else self._validate_envelope)()
        try:
            self.runtime_root, self.runtime_root_fingerprint = resolve_runtime_root(self.envelope["execution_bootstrap"]["runtime_context"])
        except ContractError as exc:
            raise ControllerError(str(exc)) from exc
        self._validate_claim()

    def _validate_manifest(self) -> None:
        needed = {"schema_version", "skill_name", "skill_version", "source_candidate_head", "root_binding_algorithm", "allowed_targets", "prohibited_transports", "files", "source_hashes", "fixture_hashes", "instruction_hashes", "skill_metadata", "skill_identity"}
        if set(self.manifest) != needed or self.manifest.get("schema_version") != "workspace_skill_bundle_manifest/v8" or (self.manifest.get("skill_name"), self.manifest.get("skill_version")) != (SKILL_NAME, SKILL_VERSION):
    ...
    def _validate_envelope(self) -> None:
        ...
        if keys != required and keys != required | {"launch_idempotency_key"} or self.envelope.get("schema_version") != "task_input_envelope/v8" or self.envelope.get("adapter_version") != "east-v5-runtime-adapter/v8" or self.envelope.get("attempt") != 1:
        ...
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v8":
        ...
        if self.envelope["run_id"] == "recovery-010-110-120-v5-1" or self.envelope["supersedes"] != ["f2c2d787-a6dc-4460-bcdc-1f71a6e27292", "72965778-39aa-441d-a1c3-f96e96ebfe32"]:
    ...
    def _validate_control_envelope(self) -> None:
        ...
        if set(self.envelope) != required or self.envelope.get("schema_version") != "launcher_control_envelope/v8" or self.envelope.get("execution_mode") != "launcher-control-preflight/v8":
        ...
        bootstrap = self.envelope["execution_bootstrap"]
        keys = {"bootstrap_version", "candidate_base_sha", "candidate_head_sha", "controller_sha256", "adapter_sha256", "runtime_context", "skill_bundle"}
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v8" or bootstrap.get("candidate_head_sha") != self.manifest["source_candidate_head"] or not _GIT.fullmatch(str(bootstrap.get("candidate_base_sha"))):
    ...
    def launcher_control_preflight(self, *, task_id: str, runtime_id: str, launcher_record: Path | None = None, launcher_fail: bool = False) -> dict[str, Any]:
        ...
        registry = ArtifactRegistry(self.runtime_root, self.envelope["issue_id"], self.envelope["run_id"], 1)
    ...
    def preflight(self, *, business: bool) -> dict[str, str]:
        ...
        result = {"claim_status": "accepted", "mode": "business-preflight" if business else "claim-preflight", "skill_name": SKILL_NAME, "skill_version": SKILL_VERSION, "skill_manifest_sha256": self.manifest_hash, "candidate_head_sha": self.manifest["source_candidate_head"], "controller_sha256": self.manifest["source_hashes"]["scripts/controller.py"], "adapter_sha256": self.manifest["source_hashes"]["east_v5/runtime/controller_core.py"], "root_binding_id": self.envelope["root_binding_id"], "runtime_root_fingerprint": self.runtime_root_fingerprint, "after_configuration_hash": after_configuration_hash}
        if business:
            result["business_operation"] = "not_started"
        return result

    def root_transport_preflight(self) -> dict[str, Any]:
        """Exchange only a control canary through the governed shared root."""
        context = self.envelope["execution_bootstrap"]["runtime_context"]
        namespace = sha256({"workspace_id": context["workspace_id"], "project_id": context["project_id"], "run_id": self.envelope["run_id"]})
        canary = self.runtime_root / ".east-v5-root-transport" / namespace / "canary.json"
        if self.envelope["target_agent_id"] == "010":
            canary.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
            canary.write_bytes(canonical_bytes({"root_fingerprint": self.runtime_root_fingerprint, "namespace": namespace}))
        else:
            value = _load(canary, "RUNTIME_ROOT_TRANSPORT_UNAVAILABLE")
            if value != {"root_fingerprint": self.runtime_root_fingerprint, "namespace": namespace}:
                _fail("RUNTIME_ROOT_TRANSPORT_FINGERPRINT_DRIFT")
        return {"status": "root_transport_ready", "target_agent_id": self.envelope["target_agent_id"], "runtime_root_fingerprint": self.runtime_root_fingerprint, "business_artifact_count": 0, "business_receipt_count": 0, "downstream_task_count": 0}
    ...
    def _receipt(self, task_id: str, runtime_id: str, input_ref: dict[str, Any] | None, output_ref: dict[str, Any], route_target: str) -> dict[str, Any]:
        value = {"schema_version": "task_execution_receipt/v8", "task_id": task_id, "runtime_id": runtime_id, "issue_id": self.envelope["issue_id"], "agent_id": self.envelope["target_agent_uuid"], "run_id": self.envelope["run_id"], "trace_id": self.envelope["trace_id"], "qa_id": self.envelope["qa_id"], "attempt": self.envelope["attempt"], "runtime_root_fingerprint": self.runtime_root_fingerprint, "input_ref": input_ref, "output_ref": output_ref, "route_target": route_target, "recovery_of": self.envelope["recovery_of"]}
    ...
    def _outbox_state(self, path: Path) -> dict[str, Any]:
        state = _load(path, "RUNTIME_LAUNCH_CREATE_OUTPUT_INVALID") if path.exists() else {"schema_version": "EAST-V5-LAUNCH-OUTBOX/v8", "entries": {}}
        if set(state) != {"schema_version", "entries"} or state["schema_version"] != "EAST-V5-LAUNCH-OUTBOX/v8" or not isinstance(state["entries"], dict):
    ...
    def _launch(self, registry: ArtifactRegistry, next_envelope: dict[str, Any], *, record: Path | None, fail: bool) -> dict[str, Any]:
        ...
        binding = {"envelope_sha256": sha256(next_envelope), "candidate_head_sha": self.manifest["source_candidate_head"], "skill_id": self.envelope["execution_bootstrap"]["skill_bundle"]["skill_id"], "runtime_root_fingerprint": self.runtime_root_fingerprint}
    ...
    def run_task(self, *, task_id: str, runtime_id: str, launcher_record: Path | None = None, launcher_fail: bool = False) -> dict[str, Any]:
        if not _UUID.fullmatch(task_id) or not _UUID.fullmatch(runtime_id):
            _fail("RUNTIME_TASK_ID_INVALID")
        registry = ArtifactRegistry(self.runtime_root, self.envelope["issue_id"], self.envelope["run_id"], self.envelope["attempt"])
