"""Self-contained v9 controller with cwd-confined launch descriptors."""
...
SKILL_NAME = "east-v5-runtime-bootstrap-v9"
SKILL_VERSION = "v9"
MATERIALIZER_VERSION = "multica_daemon_frontmatter_normalizer/v9"
KEY_DERIVATION_VERSION = "east-v5-launch-idempotency/v9"
...
class RuntimeController:
    ...
    def __init__(self, skill_root: Path, envelope: dict[str, Any], claim: dict[str, Any]):
        ...
        self.control_mode = envelope.get("execution_mode") == "launcher-control-preflight/v9"
    ...
    def _validate_manifest(self) -> None:
        ...
        if set(self.manifest) != needed or self.manifest.get("schema_version") != "workspace_skill_bundle_manifest/v9" or (self.manifest.get("skill_name"), self.manifest.get("skill_version")) != (SKILL_NAME, SKILL_VERSION):
    ...
    def _derive_launch_key(self, from_target: str, to_target: str, input_ref: dict[str, Any] | None) -> str:
        """Derive the sole acceptable launch key from the v9 canonical contract."""
    ...
    def _validate_envelope(self) -> None:
        ...
        if keys != required and keys != required | {"launch_idempotency_key"} or self.envelope.get("schema_version") != "task_input_envelope/v9" or self.envelope.get("adapter_version") != "east-v5-runtime-adapter/v9" or self.envelope.get("attempt") != 3:
        ...
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v9":
    ...
    def _validate_control_envelope(self) -> None:
        ...
        if set(self.envelope) != required or self.envelope.get("schema_version") != "launcher_control_envelope/v9" or self.envelope.get("execution_mode") != "launcher-control-preflight/v9":
        ...
        bootstrap = self.envelope["execution_bootstrap"]
        keys = {"bootstrap_version", "candidate_base_sha", "candidate_head_sha", "controller_sha256", "adapter_sha256", "runtime_context", "skill_bundle"}
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v9" or bootstrap.get("candidate_head_sha") != self.manifest["source_candidate_head"] or not _GIT.fullmatch(str(bootstrap.get("candidate_base_sha"))):
    ...
    def _outbox_state(self, path: Path) -> dict[str, Any]:
        state = _load(path, "RUNTIME_LAUNCH_CREATE_OUTPUT_INVALID") if path.exists() else {"schema_version": "EAST-V5-LAUNCH-OUTBOX/v9", "entries": {}}
        if set(state) != {"schema_version", "entries"} or state["schema_version"] != "EAST-V5-LAUNCH-OUTBOX/v9" or not isinstance(state["entries"], dict):
