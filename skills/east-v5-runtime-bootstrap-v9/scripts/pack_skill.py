#!/usr/bin/env python3
"""Build the immutable v9 installation archive from an already-committed head."""
...
_SOURCE_DESCRIPTION = b"description: Execute the EAST V5 v9 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task.\n"
_RUNTIME_DESCRIPTION = b'description: "Execute the EAST V5 v9 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task."\n'
...
def build(repo_root: Path, head: str, output: Path) -> dict[str, Any]:
    ...
    skill_prefix = "skills/east-v5-runtime-bootstrap-v9/"
    ...
    derivation_contract = {"version": "east-v5-launch-idempotency/v9", "canonical_fields": ["key_contract_version", "skill_name", "skill_version", "skill_manifest_sha256", "candidate_head_sha", "root_binding_id", "run_id", "trace_id", "qa_id", "attempt", "from", "to", "input_ref"]}
    ...
    return {"schema_version": "workspace_skill_bundle_build_receipt/v9", "skill_name": manifest["skill_name"], "skill_version": manifest["skill_version"], "source_candidate_head": head, "archive_sha256": archive_hash, "manifest_sha256": manifest_hash, "supporting_files": manifest["files"], "skill_metadata": manifest["skill_metadata"], "skill_identity": skill_identity, "key_derivation": manifest["key_derivation"]}
