#!/usr/bin/env python3
"""Build the immutable v10 installation archive from an already-committed head."""
...
_SOURCE_DESCRIPTION = b"description: Execute the EAST V5 v10 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task.\n"
_RUNTIME_DESCRIPTION = b'description: "Execute the EAST V5 v10 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task."\n'
...
def build(repo_root: Path, head: str, output: Path) -> dict[str, Any]:
    ...
    skill_prefix = "skills/east-v5-runtime-bootstrap-v10/"
    ...
    derivation_contract = {"version": "east-v5-launch-idempotency/v10", "canonical_fields": ["key_contract_version", "skill_name", "skill_version", "skill_manifest_sha256", "candidate_head_sha", "root_binding_id", "run_id", "trace_id", "qa_id", "attempt", "from", "to", "input_ref"]}
    ...
    manifest = {**template, "source_candidate_head": head, "files": {name: hashlib.sha256(payloads[name]).hexdigest() for name in relative}, "source_hashes": {"scripts/controller.py": hashlib.sha256(payloads["scripts/controller.py"]).hexdigest(), "east_v5/runtime/controller_core.py": hashlib.sha256(payloads["east_v5/runtime/controller_core.py"]).hexdigest()}, "fixture_hashes": {"fixtures/penalty/matched.json": hashlib.sha256(payloads["fixtures/penalty/matched.json"]).hexdigest()}, "instruction_hashes": instruction_hashes, "skill_metadata": skill_metadata, "skill_identity": skill_identity}
    ...
    return {"schema_version": "workspace_skill_bundle_build_receipt/v10", "skill_name": manifest["skill_name"], "skill_version": manifest["skill_version"], "source_candidate_head": head, "archive_sha256": archive_hash, "manifest_sha256": manifest_hash, "supporting_files": manifest["files"], "skill_metadata": manifest["skill_metadata"], "skill_identity": skill_identity, "key_derivation": manifest["key_derivation"]}
