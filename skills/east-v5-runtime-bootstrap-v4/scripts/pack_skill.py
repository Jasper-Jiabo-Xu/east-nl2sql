#!/usr/bin/env python3
"""Build the immutable v4 installation archive from an already-committed head."""
...
def build(repo_root: Path, head: str, output: Path) -> dict[str, Any]:
    ...
    skill_prefix = "skills/east-v5-runtime-bootstrap-v4/"
    ...
    manifest = {**template, "source_candidate_head": head, "files": {name: hashlib.sha256(payloads[name]).hexdigest() for name in relative}, "source_hashes": {"scripts/controller.py": hashlib.sha256(payloads["scripts/controller.py"]).hexdigest(), "east_v5/runtime/controller_core.py": hashlib.sha256(payloads["east_v5/runtime/controller_core.py"]).hexdigest()}, "fixture_hashes": {"fixtures/penalty/matched.json": hashlib.sha256(payloads["fixtures/penalty/matched.json"]).hexdigest()}}
    ...
    return {"schema_version": "workspace_skill_bundle_build_receipt/v4", "skill_name": manifest["skill_name"], "skill_version": manifest["skill_version"], "source_candidate_head": head, "archive_sha256": archive_hash, "manifest_sha256": manifest_hash, "supporting_files": manifest["files"]}
