"""Small standard-library-only immutable registry for the v2 controller."""
...
class ArtifactRegistry:
    def __init__(self, runtime_root: Path, issue_id: str, run_id: str, attempt: int):
        if not runtime_root.is_absolute() or attempt != 3:
    ...
    def _state(self) -> Iterator[dict[str, Any]]:
        ...
        with self.lock_path.open("a+", encoding="utf-8") as lock:
            ...
            state = json.loads(self.path.read_text(encoding="utf-8")) if self.path.exists() else {"schema_version": "EAST-V5-RUNTIME-REGISTRY/v2", "records": []}
            yield state
    ...
    def _validate(package: dict[str, Any]) -> None:
        ...
        if not required.issubset(envelope) or envelope.get("schema_version") != "COMMON-ENVELOPE/v1" or envelope.get("attempt_no") != 3 or envelope.get("status") != "candidate":
