"""Self-contained v10 controller with cwd-confined launch descriptors."""
...
SKILL_NAME = "east-v5-runtime-bootstrap-v10"
SKILL_VERSION = "v10"
MATERIALIZER_VERSION = "multica_daemon_frontmatter_normalizer/v10"
KEY_DERIVATION_VERSION = "east-v5-launch-idempotency/v10"
KEY_DERIVATION_FIELDS = [
    "key_contract_version", "skill_name", "skill_version", "skill_manifest_sha256",
    "candidate_head_sha", "root_binding_id", "run_id", "trace_id", "qa_id",
    "attempt", "from", "to", "input_ref",
]
...
class RuntimeController:
    ...
    def __init__(self, skill_root: Path, envelope: dict[str, Any], claim: dict[str, Any]):
        ...
        self.control_mode = envelope.get("execution_mode") == "launcher-control-preflight/v10"
        self._validate_manifest()
        (self._validate_control_envelope if self.control_mode else self._validate_envelope)()
        self._validate_claim()

    def _validate_manifest(self) -> None:
        needed = {"schema_version", "skill_name", "skill_version", "source_candidate_head", "root_binding_algorithm", "allowed_targets", "prohibited_transports", "files", "source_hashes", "fixture_hashes", "instruction_hashes", "skill_metadata", "skill_identity", "key_derivation"}
        if set(self.manifest) != needed or self.manifest.get("schema_version") != "workspace_skill_bundle_manifest/v10" or (self.manifest.get("skill_name"), self.manifest.get("skill_version")) != (SKILL_NAME, SKILL_VERSION):
        ...
        metadata = self.manifest.get("skill_metadata")
        if not isinstance(metadata, dict) or set(metadata) != {"name", "description", "skill_md_sha256"} or metadata.get("name") != SKILL_NAME or not isinstance(metadata.get("description"), str) or not metadata["description"] or metadata.get("skill_md_sha256") != files.get("SKILL.md"):
            _fail("RUNTIME_SKILL_METADATA_INVALID")
        ...
        identity = self.manifest.get("skill_identity")
        if not isinstance(identity, dict) or set(identity) != {"source_skill_sha256", "archive_skill_sha256", "runtime_materialized_skill_sha256", "materializer_contract"}:
            _fail("RUNTIME_SKILL_IDENTITY_INVALID")
        if identity.get("source_skill_sha256") != files.get("SKILL.md") or identity.get("archive_skill_sha256") != identity.get("source_skill_sha256") or not _HEX.fullmatch(str(identity.get("runtime_materialized_skill_sha256"))) or identity.get("runtime_materialized_skill_sha256") == identity.get("source_skill_sha256") or identity.get("materializer_contract") != {**contract, "fingerprint": hashlib.sha256(canonical_bytes(contract)).hexdigest()}:
            _fail("RUNTIME_SKILL_MATERIALIZER_CONTRACT_INVALID")
        if file_sha256(self.root / "SKILL.md") != identity["runtime_materialized_skill_sha256"]:
            _fail("RUNTIME_SKILL_MATERIALIZED_HASH_DRIFT")
        key_contract = {"version": KEY_DERIVATION_VERSION, "canonical_fields": KEY_DERIVATION_FIELDS}
        derivation = self.manifest.get("key_derivation")
        if derivation != {**key_contract, "fingerprint": hashlib.sha256(canonical_bytes(key_contract)).hexdigest()}:
            _fail("RUNTIME_KEY_DERIVATION_CONTRACT_DRIFT")

    def _derive_launch_key(self, from_target: str, to_target: str, input_ref: dict[str, Any] | None) -> str:
        """Derive the sole acceptable launch key from the v10 canonical contract."""
        return hashlib.sha256(canonical_bytes({
            "key_contract_version": KEY_DERIVATION_VERSION,
            "skill_name": SKILL_NAME,
            "skill_version": SKILL_VERSION,
            "skill_manifest_sha256": self.manifest_hash,
            "candidate_head_sha": self.manifest["source_candidate_head"],
            "root_binding_id": self.envelope["root_binding_id"],
            "run_id": self.envelope["run_id"],
            "trace_id": self.envelope["trace_id"],
            "qa_id": self.envelope["qa_id"],
            "attempt": self.envelope.get("attempt", 0),
            "from": from_target,
            "to": to_target,
            "input_ref": input_ref,
        })).hexdigest()

    def _require_derived_key(self, reported: Any, from_target: str, to_target: str, input_ref: dict[str, Any] | None) -> str:
        expected = self._derive_launch_key(from_target, to_target, input_ref)
        if not isinstance(reported, str) or reported != expected:
            _fail("RUNTIME_LAUNCH_IDEMPOTENCY_KEY_DERIVATION_DRIFT")
        return expected
    ...
    def _validate_envelope(self) -> None:
        ...
        if keys != required and keys != required | {"launch_idempotency_key"} or self.envelope.get("schema_version") != "task_input_envelope/v10" or self.envelope.get("adapter_version") != "east-v5-runtime-adapter/v10" or self.envelope.get("attempt") != 3:
        ...
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v10":
        ...
        if self.envelope["supersedes"] != ["124a3e9d-fff7-4492-8c7b-9991d018795d", "a7d894d2-1c82-4bf2-a66a-5035404a6b4c", "f2a38505-ee38-45df-ab19-79987355a299"]:
            _fail("RUNTIME_RECOVERY_SUPERSEDES_INVALID")
        reported = self.envelope.get("launch_idempotency_key")
        if self.envelope["target_agent_id"] == "010":
            if reported is not None:
                _fail("RUNTIME_LAUNCH_IDEMPOTENCY_KEY_DERIVATION_DRIFT")
        else:
            source_uuid = self.envelope["input_receipt"].get("agent_id") if isinstance(self.envelope["input_receipt"], dict) else None
            source = next((name for name, details in _TARGETS.items() if details["uuid"] == source_uuid), None)
            if source is None:
                _fail("RUNTIME_LAUNCH_IDEMPOTENCY_KEY_DERIVATION_DRIFT")
            self._require_derived_key(reported, source, self.envelope["target_agent_id"], self.envelope["input_ref"])
    ...
    def _validate_control_envelope(self) -> None:
        ...
        if set(self.envelope) != required or self.envelope.get("schema_version") != "launcher_control_envelope/v10" or self.envelope.get("execution_mode") != "launcher-control-preflight/v10":
        ...
        bootstrap = self.envelope["execution_bootstrap"]
        keys = {"bootstrap_version", "candidate_base_sha", "candidate_head_sha", "controller_sha256", "adapter_sha256", "runtime_context", "skill_bundle"}
        if not isinstance(bootstrap, dict) or set(bootstrap) != keys or bootstrap.get("bootstrap_version") != "east-v5-runtime-bootstrap/v10" or bootstrap.get("candidate_head_sha") != self.manifest["source_candidate_head"] or not _GIT.fullmatch(str(bootstrap.get("candidate_base_sha"))):
        ...
        if self.envelope["root_binding_id"] != computed:
            _fail("RUNTIME_ROOT_BINDING_DRIFT")
        self._require_derived_key(self.envelope["launch_idempotency_key"], "010", "110", None)
    ...
    def launcher_control_preflight(self, runtime_root: Path, *, task_id: str, runtime_id: str, launcher_record: Path | None = None, launcher_fail: bool = False) -> dict[str, Any]:
        ...
        registry = ArtifactRegistry(runtime_root, self.envelope["root_binding_id"], self.envelope["issue_id"], self.envelope["run_id"], 1)
        control = {**self.envelope, "target_agent_id": "110", "target_agent_uuid": _TARGETS["110"]["uuid"]}
        key, entry, control = self._prepare_launch(control)
        registry.prepare_outbox(key, entry)
        launched = self._launch(registry, control, record=launcher_record, fail=launcher_fail)
        phase = registry.outbox_entry(launched["launch_idempotency_key"])["state"]
    ...
    def _receipt(self, task_id: str, runtime_id: str, input_ref: dict[str, Any] | None, output_ref: dict[str, Any], route_target: str) -> dict[str, Any]:
        value = {"schema_version": "task_execution_receipt/v10", "task_id": task_id, "runtime_id": runtime_id, "issue_id": self.envelope["issue_id"], "agent_id": self.envelope["target_agent_uuid"], "run_id": self.envelope["run_id"], "trace_id": self.envelope["trace_id"], "qa_id": self.envelope["qa_id"], "attempt": self.envelope["attempt"], "input_ref": input_ref, "output_ref": output_ref, "route_target": route_target, "recovery_of": self.envelope["recovery_of"], "key_derivation_contract": KEY_DERIVATION_VERSION, "key_derivation_fingerprint": self.manifest["key_derivation"]["fingerprint"]}
    ...
    def _next_envelope(self, reference: dict[str, Any], receipt: dict[str, Any]) -> dict[str, Any]:
        ...
        result = {**self.envelope, "target_agent_id": next_target, "target_agent_uuid": target["uuid"], "input_ref": reference, "input_receipt": receipt, "expected_output": {"artifact_type": target["artifact_type"], "producer_id": target["producer"], "route_target": target["route"]}}
        result.pop("launch_idempotency_key", None)
        return result
    ...
    def _prepare_launch(self, next_envelope: dict[str, Any]) -> tuple[str, dict[str, Any], dict[str, Any]]:
        """Derive a launch key before its entry is committed with the artifact."""
        target = next_envelope["target_agent_id"]
        source = "010" if self.control_mode else self.envelope["target_agent_id"]
        input_ref = next_envelope.get("input_ref")
        key = self._derive_launch_key(source, target, input_ref)
        if "launch_idempotency_key" in next_envelope:
            self._require_derived_key(next_envelope["launch_idempotency_key"], source, target, input_ref)
        next_envelope = {**next_envelope, "launch_idempotency_key": key}
        binding = {"envelope_sha256": sha256(next_envelope), "candidate_head_sha": self.manifest["source_candidate_head"], "skill_id": self.envelope["execution_bootstrap"]["skill_bundle"]["skill_id"], "key_contract_version": KEY_DERIVATION_VERSION, "key_derivation_fingerprint": self.manifest["key_derivation"]["fingerprint"]}
        return key, {"state": "published", "target": target, "envelope": next_envelope, **binding}, next_envelope

    def _launch(self, registry: ArtifactRegistry, next_envelope: dict[str, Any], *, record: Path | None, fail: bool) -> dict[str, Any]:
        """Launch only a transactionally published, independently resolvable entry."""
        key, expected_entry, next_envelope = self._prepare_launch(next_envelope)
        target = next_envelope["target_agent_id"]
        source = "010" if self.control_mode else self.envelope["target_agent_id"]
        input_ref = next_envelope.get("input_ref")
        entry = registry.outbox_entry(key)
        binding = {name: value for name, value in expected_entry.items() if name != "state"}
        if entry.get("state") not in {"published", "issue_id", "task_id"} or any(entry.get(name) != value for name, value in binding.items()):
            _fail("RUNTIME_LAUNCH_OUTBOX_BINDING_DRIFT")
        if fail:
            registry.abort_outbox(key)
        ...
        if record is not None:
            ...
            registry.update_outbox(key, state="task_id", issue_id=result["issue_id"], task_id=result["task_id"])
            return result
        if entry.get("issue_id") is None:
            # Recompute immediately before the external create side effect.
            self._require_derived_key(next_envelope["launch_idempotency_key"], source, target, input_ref)
            ...
            try:
                ...
                if not isinstance(issue_id, str) or not issue_id:
                    _fail("RUNTIME_LAUNCH_CREATE_OUTPUT_INVALID")
                entry = registry.update_outbox(key, state="issue_id", issue_id=issue_id)
        ...
        try:
            for _ in range(3):
                ...
                if isinstance(task_id, str) and task_id:
                    registry.update_outbox(key, state="task_id", task_id=task_id)
    ...
    def _cross_process_resolve(self, runtime_root: Path, reference: dict[str, Any], task_id: str, receipt: dict[str, Any]) -> None:
        """Read the committed publication through a fresh interpreter before launch."""
        cwd, descriptor = self._descriptor(reference["content_hash"], {"reference": reference, "task_id": task_id})
        try:
            command = [sys.executable, "-I", str(self.root / "scripts" / "controller.py"), "resolve-preflight", "--runtime-root", str(runtime_root), "--root-binding-id", self.envelope["root_binding_id"], "--issue-id", self.envelope["issue_id"], "--run-id", self.envelope["run_id"], "--attempt", str(self.envelope["attempt"]), "--reference-file", str(descriptor.relative_to(cwd)), "--receipt-task-id", task_id]
            check = subprocess.run(command, check=True, capture_output=True, text=True, cwd=str(cwd))
            value = json.loads(check.stdout)
            if value.get("reference") != reference or value.get("receipt") != receipt:
                _fail("RUNTIME_CROSS_PROCESS_RESOLUTION_REJECTED")
        except (OSError, subprocess.SubprocessError, json.JSONDecodeError) as exc:
            raise ControllerError("RUNTIME_CROSS_PROCESS_RESOLUTION_REJECTED") from exc
        finally:
            descriptor.unlink(missing_ok=True)
    ...
    def run_task(self, runtime_root: Path, *, task_id: str, runtime_id: str, launcher_record: Path | None = None, launcher_fail: bool = False) -> dict[str, Any]:
        ...
        registry = ArtifactRegistry(runtime_root, self.envelope["root_binding_id"], self.envelope["issue_id"], self.envelope["run_id"], self.envelope["attempt"])
        prior = registry.journal_entry(task_id)
        if prior is not None and prior.get("stage") == "committed":
            return prior
        ...
        if _TARGETS[target]["route"] != "complete":
            ...
            key, entry, next_envelope = self._prepare_launch(next_envelope)
        published = {"output_ref": output_ref, "receipt": receipt, "next_task": None, "next_key": key, "stage": "published"}
        if prior is None:
            registry.publish(package=package if target in {"010", "120"} else None, receipt=receipt, task_id=task_id, outbox_key=key, outbox_entry=entry, result=published)
        elif prior != published:
            _fail("RUNTIME_EXECUTION_JOURNAL_CONFLICT")
        self._cross_process_resolve(runtime_root, output_ref, task_id, receipt)
        launched = self._launch(registry, next_envelope, record=launcher_record, fail=launcher_fail) if next_envelope is not None else None
        result = {"output_ref": output_ref, "receipt": receipt, "next_task": launched, "stage": "committed"}
        return registry.finalize(task_id, result)
