#!/usr/bin/env python3
"""Build the immutable v12 full-graph installation archive from a committed head."""
...
MATERIALIZER_VERSION = "multica_daemon_frontmatter_normalizer/v12"
_SOURCE_DESCRIPTION = b"description: Execute the EAST V5 v12 full-graph controller for all 17 real Agents and the fixed 000 component, only after a complete fail-closed preflight.\n"
_RUNTIME_DESCRIPTION = b'description: "Execute the EAST V5 v12 full-graph controller for all 17 real Agents and the fixed 000 component, only after a complete fail-closed preflight."\n'


def materialize_skill(skill_md: bytes) -> bytes:
    """Apply the only approved daemon normalization byte-for-byte."""
    if skill_md.count(_SOURCE_DESCRIPTION) != 1 or skill_md.count(b"---\n\n# EAST V5 Runtime Bootstrap V12") != 1:
...
def build(repo_root: Path, head: str, output: Path) -> dict[str, Any]:
    ...
    derivation_contract = {"version": "east-v5-launch-idempotency/v12", "canonical_fields": ["skill_name", "skill_version", "skill_manifest_sha256", "root_binding_id", "run_id", "attempt", "target_agent_id", "input_receipt_hashes"]}
    ...
    return {"schema_version": "workspace_skill_bundle_build_receipt/v12", "skill_name": manifest["skill_name"], "skill_version": manifest["skill_version"], "source_candidate_head": head, "archive_sha256": archive_hash, "manifest_sha256": manifest_hash, "supporting_files": manifest["files"], "skill_metadata": manifest["skill_metadata"], "skill_identity": skill_identity, "key_derivation": manifest["key_derivation"]}
