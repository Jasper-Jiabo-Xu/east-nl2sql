...
- `tests/runtime/test_full_runtime_graph_v11.py`：真实控制器打包后执行 event 全图、Foundation 独立链、双审与数据/ORM 双屏障、17/17 零任务预检拒绝、未知/重复 receipt 拒绝、同 task 重放和 envelope 漂移拒绝、instruction/preflight 漂移拒绝且零进度、受影响链重试、第三次失败 terminal 收口；测试复制项目 `.gitignore` 后再提交/打包，覆盖 runtime 源码误忽略回归。
...
| v11 合同测试 | `1cabac1ce58c3f5fa9eed8d31f21df1d3b4e08973b872c5b9054f0bfb5f119d9` |
