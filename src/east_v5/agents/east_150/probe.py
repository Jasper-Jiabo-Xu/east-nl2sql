...
def run_sanitized_probe(root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        release = _wrap("release_candidate", "release210-eas24", {"synthetic": "release"}, "210")
        receipt = _wrap("release_receipt", "receipt010-eas24", {"schema_version": "v5.release-receipt/v1", "candidate_sha256": "a" * 64, "released_version": "candidate", "committed_package_sha256": "b" * 64, "approved_by": "probe"}, "010", parents=[artifact_ref(release["envelope"])])
        registry.register(release["envelope"], release["payload"]); registry.register(receipt["envelope"], receipt["payload"])
        capability_regression = TrustedRouteCapability.from_registry(registry, release_candidate_ref=artifact_ref(release["envelope"]), release_receipt_ref=artifact_ref(receipt["envelope"]))
        ...
        regression = _regression(parents=[artifact_ref(receipt["envelope"])])
        ...
        route_rejections = {"170_forged_prefix": _is_rejected(lambda: builder.handle_routed_feedback(query, _review(first, parents=[_ref("110")]), first, route_capability=capability_170, run_id="eas24-probe", qa_id="QA-EAS24", attempt_no=2, created_at=TIME, **_candidate("SELECT T1.F2 FROM T1"))), "260_missing_010": _is_rejected(lambda: builder.handle_routed_feedback(query, _regression(parents=[artifact_ref(release["envelope"])]), reviewed, route_capability=capability_regression, run_id="eas24-probe", qa_id="QA-EAS24", attempt_no=2, created_at=TIME, **_candidate("SELECT T1.F2 FROM T1"))), "260_candidate_previous": _is_rejected(lambda: builder.handle_routed_feedback(query, regression, first, route_capability=capability_regression, run_id="eas24-probe", qa_id="QA-EAS24", attempt_no=2, created_at=TIME, **_candidate("SELECT T1.F2 FROM T1")))}
