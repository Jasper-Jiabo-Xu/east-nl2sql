...
def run_sanitized_probe(root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        extra_parent_record = _wrap("sql_regression_route_record", "route110-extra-parent", route_record["payload"], "110", mode="event_data", parents=[artifact_ref(regression["envelope"]), artifact_ref(query["envelope"])])
        wrong_mode_record = _wrap("sql_regression_route_record", "route110-wrong-mode", route_record["payload"], "110", mode="question_sql", parents=[artifact_ref(regression["envelope"])])
        wrong_attempt_record = _wrap("sql_regression_route_record", "route110-wrong-attempt", route_record["payload"], "110", attempt=2, mode="event_data", parents=[artifact_ref(regression["envelope"])])
        malformed_feedback = copy.deepcopy(regression); malformed_feedback["envelope"]["artifact_id"] = "regression024-malformed"; malformed_feedback["payload"].pop("sandbox_snapshot_id"); malformed_feedback["envelope"]["content_hash"] = content_hash(malformed_feedback["envelope"], malformed_feedback["payload"])
        registry.register(malformed_feedback["envelope"], malformed_feedback["payload"])
        malformed_record_payload = {**route_record["payload"], "source_feedback_ref": artifact_ref(malformed_feedback["envelope"])}
        malformed_record = _wrap("sql_regression_route_record", "route110-malformed-source", malformed_record_payload, "110", mode="event_data", parents=[artifact_ref(malformed_feedback["envelope"])])
        for record in (extra_parent_record, wrong_mode_record, wrong_attempt_record, malformed_record):
            registry.register(record["envelope"], record["payload"])
        ...
        mint_rejections = {"extra_parent": _is_rejected(lambda: TrustedRouteCapability.from_registry(registry, route_record_ref=artifact_ref(extra_parent_record["envelope"]))), "wrong_mode": _is_rejected(lambda: TrustedRouteCapability.from_registry(registry, route_record_ref=artifact_ref(wrong_mode_record["envelope"]))), "wrong_attempt": _is_rejected(lambda: TrustedRouteCapability.from_registry(registry, route_record_ref=artifact_ref(wrong_attempt_record["envelope"]))), "malformed_feedback_at_mint": _is_rejected(lambda: TrustedRouteCapability.from_registry(registry, route_record_ref=artifact_ref(malformed_record["envelope"])))}
