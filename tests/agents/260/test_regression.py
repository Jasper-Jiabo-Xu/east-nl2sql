...
class EventRegressionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.spec = wrap("query_specification_package", "qspec-260", query_payload, "140", "event_data", status="validated"); self.spec["envelope"]["qa_id"] = "QA-260"; self.spec["envelope"]["content_hash"] = content_hash(self.spec["envelope"], self.spec["payload"])
    ...
    def test_orm_execution_and_gold_sql_errors_have_distinct_feedback(self):
        ...
        orm_feedback = self.worker.run_event(self.data, self.orm, self.snapshot, self.review, self.spec, self.db)
    ...
    def test_110_unknown_field_and_schema_drift_are_rejected(self):
        ...
        with self.assertRaisesRegex(ContractError, "SCHEMA_VALIDATION_FAILED:QUESTION_SQL_DUAL_REVIEW_PASSED"):
            self.worker.run_event(self.data,self.orm,self.snapshot,self.review,self.spec,self.db)
