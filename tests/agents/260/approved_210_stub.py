...
def consume(package: dict[str, Any], root: Path) -> dict[str, str]:
    ...
    if artifact_type == "database_copy_regression":
        ...
        if mode == "event_data" and (len(refs) != 7 or package["payload"]["query_spec_ref"] not in refs or package["payload"].get("event_query_context_ref") not in refs or package["payload"].get("query_parameter_binding_ref") not in refs or not package["payload"].get("query_parameter_binding_hash")):
