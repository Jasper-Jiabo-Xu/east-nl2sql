...
def event_material(*, duplicate: bool = False) -> tuple[dict, dict, dict]:
    ...
    approved_payload["candidate_content"]["query_parameter_bindings"] = []
    ...
    reviewed_ref, context_ref, binding_ref = ref("reviewed-question-sql", "6"), ref("event-query-context", "7"), ref("query-parameter-binding", "8")
    ...
    regression_payload = {"schema_version": "v5.regression-passed-data-orm/v2", "regression_package_id": "event-regression", "mode": "event_data", "data_package_refs": [data_ref], "orm_plan_ref": orm_ref, "question_sql_ref": artifact_ref(approved["envelope"]), "reviewed_question_sql_ref": reviewed_ref, "event_query_context_ref": context_ref, "query_parameter_binding_ref": binding_ref, "query_parameter_binding_hash": binding_ref["content_hash"], "query_spec_ref": approved_payload["query_specification_package"], "execution_instances": {"orm_params": {"fixture": "e-1"}, "query_binding_names": [], "operations": ["insert"]}, "sandbox_snapshot_id": "snapshot-1", "sandbox_execution_report": {"operations": writes, "write_count": len(writes), "rolled_back": False}, "sql_regression_report": {"sql_gold": "SELECT value FROM EVENT_RECORD", "row_count": 1, "positive_negative_metrics": {"positive_hits": 1, "minimum_positive_count": 1, "negative_fixture_count": 1, "minimum_negative_count": 1, "negative_hits": 0, "negative_excluded": True, "condition_coverage_passed": True, "code_value_coverage_passed": True, "passed": True}, "density_group_metrics": {"row_count": 1, "distinct_count": 1, "group_count": 1, "target": 1, "tolerance_range": {"low": 1, "high": 1}, "distinct_required": True, "passed": True}, "join_expansion_metrics": {"row_count": 1, "baseline_grain_count": 1, "actual_multiplier": 1.0, "max_multiplier": 1.0, "max_result_rows": 1, "passed": True}}, "executable_package_hash": "5" * 64, "regression_status": "passed", "regressed_at": TIME}
    regression = package("database_copy_regression", regression_payload, producer="260", mode="event_data", parents=[reviewed_ref, context_ref, binding_ref])
    candidate_payload = {"schema_version": "v5.release-candidate/v2", "release_candidate_id": "event-release", "release_mode": "event_data", "approved_question_sql_ref": artifact_ref(approved["envelope"]), "event_regression_passed_ref": artifact_ref(regression["envelope"]), "foundation_regression_report_ref": None, "target_database_version": "fixture-db-v1", "target_question_dataset_version": "fixture-question-v1", "idempotency_key": "event-idempotency-1", "expected_write_summary": {"orm_execution": {"insert_or_update": len(writes)}}, "package_hashes": {"question_sql": approved["envelope"]["content_hash"], "data": data_ref["content_hash"], "orm": orm_ref["content_hash"], "query_binding": binding_ref["content_hash"], "regression": regression["envelope"]["content_hash"]}, "resume_qa_ref": None}
...
class FormalReleaseCommitterTests(unittest.TestCase):
    ...
    def test_query_parameter_binding_execution_evidence_uses_the_same_010_to_110_recovery_path(self) -> None:
        payload = {"schema_version": "v5.sql-regression-failed-feedback/v1", "mode": "event_data", "input_data_refs": [ref("data", "a")], "input_orm_ref": ref("orm", "b"), "sandbox_snapshot_id": "snapshot", "failure_details": {"error_code": "QUERY_PARAMETER_BINDING_ERROR", "error_stage": "binding", "error_location": "database_copy", "expected_values": ["needle"], "actual_values": ["driver rejected binding"], "sql_error_detail": {"sql_text": "SELECT :needle", "error_code": "SQLITE_ERROR", "error_message": "binding parameter unsupported"}, "regression_metrics": {}}, "route_target": "010", "retry_count": 1}
        feedback = package("sql_regression_failed_feedback", payload, producer="260", mode="event_data")
        self.assertEqual(self.committer.route_sql_regression_failure(feedback)["kind"], "query_parameter_binding_repair")
