...
SOURCE = ROOT / "skills" / "east-v5-runtime-bootstrap-v9"
...
def _materialize(path: Path) -> None:
    value = path.read_bytes().replace(
        b"description: Execute the EAST V5 v9 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task.\n",
        b'description: "Execute the EAST V5 v9 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task."\n',
        1,
    ).replace(b"---\n\n# EAST V5 Runtime Bootstrap V9", b"---\n\n\n# EAST V5 Runtime Bootstrap V9", 1)
    path.write_bytes(value)


class RuntimeSkillV9Tests(unittest.TestCase):
    def _archive(self, directory: Path) -> tuple[Path, dict[str, object], Path]:
        repo = directory / "repo"
        skill = repo / "skills" / "east-v5-runtime-bootstrap-v9"
        ...
        subprocess.run(["git", "-C", str(repo), "-c", "user.email=test@example.invalid", "-c", "user.name=test", "commit", "-qm", "v9"], check=True)
        head = subprocess.check_output(["git", "-C", str(repo), "rev-parse", "HEAD"], text=True).strip()
        archive = directory / "v9.skill.zip"
    ...
    def _control(self, installed: Path, receipt: dict[str, object], target: str = "010", key_seed: bytes = b"v9-real-control-preflight") -> dict[str, object]:
        business = self._envelope(installed, receipt)
        manifest = json.loads((installed / "manifest.json").read_text(encoding="utf-8"))
        contract = {"key_contract_version": "east-v5-launch-idempotency/v9", "skill_name": "east-v5-runtime-bootstrap-v9", "skill_version": "v9", "skill_manifest_sha256": _sha(installed / "manifest.json"), "candidate_head_sha": manifest["source_candidate_head"], "root_binding_id": business["root_binding_id"], "run_id": "launcher-control-v9", "trace_id": "launcher-control-trace", "qa_id": "EAS70-LAUNCHER-CONTROL", "attempt": 0, "from": "010", "to": "110", "input_ref": None}
        key = hashlib.sha256(json.dumps(contract, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
        control = {"schema_version": "launcher_control_envelope/v9", "execution_mode": "launcher-control-preflight/v9", "issue_id": business["issue_id"], "platform_parent_issue_id": business["platform_parent_issue_id"], "project_id": business["project_id"], "run_id": "launcher-control-v9", "trace_id": "launcher-control-trace", "qa_id": "EAS70-LAUNCHER-CONTROL", "target_agent_id": target, "target_agent_uuid": TARGETS[target][0], "root_binding_id": business["root_binding_id"], "launch_idempotency_key": key, "execution_bootstrap": business["execution_bootstrap"], "callback": {"callback_agent_id": "1cdd93d3-b5fa-4dae-9b09-8320055c3072", "callback_issue_id": "42f91f92-9453-4e7a-a38e-30d629ae07d6", "callback_condition": "consume control task"}}
        if key_seed != b"v9-real-control-preflight":
    ...
    def test_packager_is_deterministic_and_binds_final_head(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["skill_metadata"]["name"], "east-v5-runtime-bootstrap-v9")
            ...
            self.assertEqual(manifest["key_derivation"]["version"], "east-v5-launch-idempotency/v9")
    ...
    def test_isolated_full_chain_and_idempotent_replay(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(out_010["receipt"]["key_derivation_contract"], "east-v5-launch-idempotency/v9")
    ...
    def test_metadata_materialization_drift_rejects_before_business_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (installed / "SKILL.md").write_text(original.replace("name: east-v5-runtime-bootstrap-v9", "name: east-v5-runtime-bootstrap-v9-skill", 1), encoding="utf-8")
