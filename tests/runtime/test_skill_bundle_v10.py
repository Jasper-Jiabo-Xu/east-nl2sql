...
SOURCE = ROOT / "skills" / "east-v5-runtime-bootstrap-v10"
PACKAGER = SOURCE / "scripts" / "pack_skill.py"

SKILL_ID = "99999999-8888-4777-8666-555555555555"
TARGETS = {
...
def _materialize(path: Path) -> None:
    value = path.read_bytes().replace(
        b"description: Execute the EAST V5 v10 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task.\n",
        b'description: "Execute the EAST V5 v10 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task."\n',
        1,
    ).replace(b"---\n\n# EAST V5 Runtime Bootstrap V10", b"---\n\n\n# EAST V5 Runtime Bootstrap V10", 1)
    path.write_bytes(value)


class RuntimeSkillV10Tests(unittest.TestCase):
    def _archive(self, directory: Path) -> tuple[Path, dict[str, object], Path]:
        repo = directory / "repo"
        skill = repo / "skills" / "east-v5-runtime-bootstrap-v10"
        ...
        subprocess.run(["git", "-C", str(repo), "-c", "user.email=test@example.invalid", "-c", "user.name=test", "commit", "-qm", "v10"], check=True)
        head = subprocess.check_output(["git", "-C", str(repo), "rev-parse", "HEAD"], text=True).strip()
        archive = directory / "v10.skill.zip"
    ...
    def _control(self, installed: Path, receipt: dict[str, object], target: str = "010", key_seed: bytes = b"v10-real-control-preflight") -> dict[str, object]:
        business = self._envelope(installed, receipt)
        manifest = json.loads((installed / "manifest.json").read_text(encoding="utf-8"))
        contract = {"key_contract_version": "east-v5-launch-idempotency/v10", "skill_name": "east-v5-runtime-bootstrap-v10", "skill_version": "v10", "skill_manifest_sha256": _sha(installed / "manifest.json"), "candidate_head_sha": manifest["source_candidate_head"], "root_binding_id": business["root_binding_id"], "run_id": "launcher-control-v10", "trace_id": "launcher-control-trace", "qa_id": "EAS70-LAUNCHER-CONTROL", "attempt": 0, "from": "010", "to": "110", "input_ref": None}
        key = hashlib.sha256(json.dumps(contract, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
        control = {"schema_version": "launcher_control_envelope/v10", "execution_mode": "launcher-control-preflight/v10", "issue_id": business["issue_id"], "platform_parent_issue_id": business["platform_parent_issue_id"], "project_id": business["project_id"], "run_id": "launcher-control-v10", "trace_id": "launcher-control-trace", "qa_id": "EAS70-LAUNCHER-CONTROL", "target_agent_id": target, "target_agent_uuid": TARGETS[target][0], "root_binding_id": business["root_binding_id"], "launch_idempotency_key": key, "execution_bootstrap": business["execution_bootstrap"], "callback": {"callback_agent_id": "1cdd93d3-b5fa-4dae-9b09-8320055c3072", "callback_issue_id": "42f91f92-9453-4e7a-a38e-30d629ae07d6", "callback_condition": "consume control task"}}
        if key_seed != b"v10-real-control-preflight":
            control["launch_idempotency_key"] = hashlib.sha256(key_seed).hexdigest()
        return control
    ...
    def _call(self, installed: Path, command: str, envelope: dict[str, object], claim: dict[str, object], directory: Path, *extra: str) -> subprocess.CompletedProcess[str]:
        ...
        if "--runtime-root" in extra:
            runtime = Path(extra[extra.index("--runtime-root") + 1])
            runtime.mkdir(mode=0o700, parents=True, exist_ok=True)
            os.chmod(runtime, 0o700)
            marker = runtime / "daemon-root-binding-v10.json"
            marker.write_text(json.dumps({"schema_version": "east-v5-daemon-root-binding/v10", "root_binding_id": envelope["root_binding_id"]}, sort_keys=True), encoding="utf-8")
    ...
    def test_packager_is_deterministic_and_binds_final_head(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["skill_metadata"]["name"], "east-v5-runtime-bootstrap-v10")
            ...
            self.assertEqual(receipt["key_derivation"], manifest["key_derivation"])
            self.assertEqual(manifest["key_derivation"]["version"], "east-v5-launch-idempotency/v10")
    ...
    def test_isolated_full_chain_and_idempotent_replay(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(out_010["receipt"]["key_derivation_contract"], "east-v5-launch-idempotency/v10")
            self.assertEqual(out_010["receipt"]["key_derivation_fingerprint"], json.loads((installed / "manifest.json").read_text(encoding="utf-8"))["key_derivation"]["fingerprint"])
    ...
    def test_registry_conflict_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(first.returncode, 0, first.stderr)
            registry_path = runtime / "east-v5-daemon-artifacts-v10" / initial["root_binding_id"] / "issues" / "EAS-70" / "recovery-probe" / "3" / "artifact-publication-v10.json"
    ...
    def test_metadata_materialization_drift_rejects_before_business_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (installed / "SKILL.md").write_text(original.replace("name: east-v5-runtime-bootstrap-v10", "name: east-v5-runtime-bootstrap-v10-skill", 1), encoding="utf-8")
    ...
    def test_launcher_failure_leaves_only_staged_nonconsumer_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            base = runtime / "east-v5-daemon-artifacts-v10" / self._envelope(installed, receipt)["root_binding_id"] / "issues" / "EAS-70" / "recovery-probe" / "3"
            publication = json.loads((base / "artifact-publication-v10.json").read_text(encoding="utf-8"))
            self.assertEqual(len(publication["records"]), 1)
            self.assertEqual(len(publication["receipts"]), 1)
            self.assertEqual(publication["outbox"], {})
            self.assertEqual(next(iter(publication["journal"].values()))["stage"], "published")
    ...
    def test_different_control_keys_do_not_share_issue(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _archive, receipt, installed = self._archive(directory)
            runtime, launches = directory / "runtime", directory / "launches.json"
            for key in ("dd5fc0f2" + "0" * 56, "774d253e" + "0" * 56):
                control = self._control(installed, receipt); control["launch_idempotency_key"] = key
                rejected = self._call(installed, "launcher-control-preflight", control, self._claim("010", receipt), directory, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000010", "--runtime-id", "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", "--launcher-record", str(launches))
                self.assertEqual(rejected.returncode, 2)
                self.assertIn("RUNTIME_LAUNCH_IDEMPOTENCY_KEY_DERIVATION_DRIFT", rejected.stderr)
            self.assertFalse((runtime / "east-v5-daemon-artifacts-v10").exists())
            self.assertFalse(launches.exists())

    def test_old_business_keys_envelopes_and_hashes_fail_before_outbox(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp)
            _archive, receipt, installed = self._archive(directory)
            runtime, launches = directory / "runtime", directory / "launches.json"
            for old_version in ("v6", "v7", "v8", "v9"):
                envelope = self._envelope(installed, receipt)
                envelope["schema_version"] = f"task_input_envelope/{old_version}"
                envelope["adapter_version"] = f"east-v5-runtime-adapter/{old_version}"
                rejected = self._call(installed, "run-task", envelope, self._claim("010", receipt), directory, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000010", "--runtime-id", "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", "--launcher-record", str(launches))
                self.assertEqual(rejected.returncode, 2)
                self.assertIn("RUNTIME_TASK_INPUT_INVALID", rejected.stderr)
            for field in ("controller_sha256", "adapter_sha256", "skill_manifest_sha256"):
                envelope = self._envelope(installed, receipt)
                if field == "skill_manifest_sha256":
                    envelope["execution_bootstrap"]["skill_bundle"][field] = "0" * 64
                else:
                    envelope["execution_bootstrap"][field] = "0" * 64
                rejected = self._call(installed, "run-task", envelope, self._claim("010", receipt), directory, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000010", "--runtime-id", "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", "--launcher-record", str(launches))
                self.assertEqual(rejected.returncode, 2)
            self.assertFalse((runtime / "east-v5-daemon-artifacts-v10").exists())
            self.assertFalse(launches.exists())

    def test_derived_business_key_replays_across_processes_and_old_keys_reject(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp)
            _archive, receipt, installed = self._archive(directory)
            runtime, launches = directory / "runtime", directory / "launches.json"
            initial = self._envelope(installed, receipt)
            first = self._call(installed, "run-task", initial, self._claim("010", receipt), directory, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000010", "--runtime-id", "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", "--launcher-record", str(launches))
            self.assertEqual(first.returncode, 0, first.stderr)
            launched = json.loads(first.stdout)["next_task"]["envelope"]
            replay = self._call(installed, "run-task", launched, self._claim("110", receipt), directory, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000110", "--runtime-id", "bbbbbbbb-cccc-4ddd-8eee-ffffffffffff", "--launcher-record", str(launches))
            self.assertEqual(replay.returncode, 0, replay.stderr)
            second = self._call(installed, "run-task", launched, self._claim("110", receipt), directory, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000110", "--runtime-id", "bbbbbbbb-cccc-4ddd-8eee-ffffffffffff", "--launcher-record", str(launches))
            self.assertEqual(second.returncode, 0, second.stderr)
            self.assertEqual(len(json.loads(launches.read_text(encoding="utf-8"))["launches"]), 2)
            for key in ("6a2298ba" + "0" * 56, "b0f40fb9" + "0" * 56):
                bad = dict(launched); bad["launch_idempotency_key"] = key
                rejected = self._call(installed, "run-task", bad, self._claim("110", receipt), directory, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000112", "--runtime-id", "bbbbbbbb-cccc-4ddd-8eee-ffffffffffff", "--launcher-record", str(launches))
                self.assertEqual(rejected.returncode, 2)
                self.assertIn("RUNTIME_LAUNCH_IDEMPOTENCY_KEY_DERIVATION_DRIFT", rejected.stderr)
            self.assertEqual(len(json.loads(launches.read_text(encoding="utf-8"))["launches"]), 2)

    def test_different_task_workdirs_consume_the_same_daemon_publication(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp)
            _archive, receipt, installed = self._archive(directory)
            runtime, launches = directory / "daemon-root", directory / "launches.json"
            task010, task110 = directory / "task-010", directory / "task-110"
            task010.mkdir(); task110.mkdir()
            initial = self._envelope(installed, receipt)
            produced = self._call(installed, "run-task", initial, self._claim("010", receipt), task010, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000010", "--runtime-id", "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", "--launcher-record", str(launches))
            self.assertEqual(produced.returncode, 0, produced.stderr)
            next_envelope = json.loads(produced.stdout)["next_task"]["envelope"]
            consumed = self._call(installed, "run-task", next_envelope, self._claim("110", receipt), task110, "--runtime-root", str(runtime), "--task-id", "00000000-0000-4000-8000-000000000110", "--runtime-id", "bbbbbbbb-cccc-4ddd-8eee-ffffffffffff", "--launcher-record", str(launches))
            self.assertEqual(consumed.returncode, 0, consumed.stderr)
            self.assertEqual(json.loads(consumed.stdout)["receipt"]["input_ref"], next_envelope["input_ref"])
