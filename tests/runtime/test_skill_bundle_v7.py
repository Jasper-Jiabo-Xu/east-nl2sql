...
SOURCE = ROOT / "skills" / "east-v5-runtime-bootstrap-v7"
...
class RuntimeSkillV7Tests(unittest.TestCase):
    def _archive(self, directory: Path) -> tuple[Path, dict[str, object], Path]:
        repo = directory / "repo"
        skill = repo / "skills" / "east-v5-runtime-bootstrap-v7"
        ...
        subprocess.run(["git", "-C", str(repo), "-c", "user.email=test@example.invalid", "-c", "user.name=test", "commit", "-qm", "v7"], check=True)
        head = subprocess.check_output(["git", "-C", str(repo), "rev-parse", "HEAD"], text=True).strip()
        archive = directory / "v7.skill.zip"
    ...
    def _control(self, installed: Path, receipt: dict[str, object], target: str = "010", key_seed: bytes = b"v7-real-control-preflight") -> dict[str, object]:
        business = self._envelope(installed, receipt)
        key = hashlib.sha256(key_seed).hexdigest()
        return {"schema_version": "launcher_control_envelope/v1", "execution_mode": "launcher-control-preflight/v1", "issue_id": business["issue_id"], "platform_parent_issue_id": business["platform_parent_issue_id"], "project_id": business["project_id"], "run_id": "launcher-control-v7", "trace_id": "launcher-control-trace", "qa_id": "EAS70-LAUNCHER-CONTROL", "target_agent_id": target, "target_agent_uuid": TARGETS[target][0], "root_binding_id": business["root_binding_id"], "launch_idempotency_key": key, "execution_bootstrap": business["execution_bootstrap"], "callback": {"callback_agent_id": "1cdd93d3-b5fa-4dae-9b09-8320055c3072", "callback_issue_id": "42f91f92-9453-4e7a-a38e-30d629ae07d6", "callback_condition": "consume control task"}}
    ...
    def test_packager_is_deterministic_and_binds_final_head(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["skill_metadata"], {"name": "east-v5-runtime-bootstrap-v7", "description": "Execute the EAST V5 v7 instruction-bound controller for 010, 110, and 120: accepted preflight must be followed by same-task committed run-task.", "skill_md_sha256": _sha(installed / "SKILL.md")})
    ...
    def test_metadata_materialization_drift_rejects_before_business_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (installed / "SKILL.md").write_text(original.replace("name: east-v5-runtime-bootstrap-v7", "name: east-v5-runtime-bootstrap-v7-skill", 1), encoding="utf-8")
            ...
            self.assertIn("RUNTIME_SKILL_SUPPORT_FILE_HASH_DRIFT", rejected.stderr)
