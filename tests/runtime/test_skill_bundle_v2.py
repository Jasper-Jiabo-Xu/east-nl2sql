...
SOURCE = ROOT / "skills" / "east-v5-runtime-bootstrap-v2"
...
class RuntimeSkillV2Tests(unittest.TestCase):
    def _archive(self, directory: Path) -> tuple[Path, dict[str, object], Path]:
        repo = directory / "repo"
        skill = repo / "skills" / "east-v5-runtime-bootstrap-v2"
        skill.parent.mkdir(parents=True)
        shutil.copytree(SOURCE, skill)
        subprocess.run(["git", "init", "-q", str(repo)], check=True)
        subprocess.run(["git", "-C", str(repo), "add", "."], check=True)
        subprocess.run(["git", "-C", str(repo), "-c", "user.email=test@example.invalid", "-c", "user.name=test", "commit", "-qm", "v2"], check=True)
        head = subprocess.check_output(["git", "-C", str(repo), "rev-parse", "HEAD"], text=True).strip()
        archive = directory / "v2.skill.zip"
    ...
    def _envelope(self, installed: Path, receipt: dict[str, object], target: str = "010", input_ref: dict[str, object] | None = None) -> dict[str, object]:
        ...
        return {"schema_version": "task_input_envelope/v2", "adapter_version": "east-v5-runtime-adapter/v2", "issue_id": "EAS-70", "platform_parent_issue_id": "42f91f92-9453-4e7a-a38e-30d629ae07d6", "project_id": "3495da30-28d4-4cc6-b249-1e186ade6872", "run_id": "v2-probe", "trace_id": "v2-trace", "qa_id": "EAS70-V2", "attempt": 3, "target_agent_id": target, "target_agent_uuid": TARGETS[target][0], "root_binding_id": binding, "input_ref": input_ref, "input_receipt": None, "expected_output": {"artifact_type": route[0], "producer_id": route[1], "route_target": route[2]}, "execution_bootstrap": {"bootstrap_version": "east-v5-runtime-bootstrap/v2", "candidate_base_sha": "9bdf86b1c5f369a8ee05bb94ba2e47aeb04414de", "candidate_head_sha": manifest["source_candidate_head"], "controller_sha256": manifest["source_hashes"]["scripts/controller.py"], "adapter_sha256": manifest["source_hashes"]["east_v5/runtime/controller_core.py"], "runtime_context": context, "skill_bundle": {"skill_name": "east-v5-runtime-bootstrap-v2", "skill_version": "v2", "skill_id": SKILL_ID, "skill_manifest_sha256": _sha(installed / "manifest.json"), "archive_sha256": receipt["archive_sha256"]}}}
    ...
    def test_registry_conflict_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(first.returncode, 0, first.stderr)
            registry_path = runtime / "east-v5-runtime" / "issues" / "EAS-70" / "v2-probe" / "3" / "artifact-registry.json"
    ...
    def test_rejects_out_of_sequence_input_and_launcher_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(launch_fail.returncode, 2); self.assertIn("RUNTIME_LAUNCH_FAILED", launch_fail.stderr)
