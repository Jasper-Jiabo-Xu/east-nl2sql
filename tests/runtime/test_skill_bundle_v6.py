...
SOURCE = ROOT / "skills" / "east-v5-runtime-bootstrap-v6"
...
class RuntimeSkillV6Tests(unittest.TestCase):
    def _archive(self, directory: Path) -> tuple[Path, dict[str, object], Path]:
        repo = directory / "repo"
        skill = repo / "skills" / "east-v5-runtime-bootstrap-v6"
        ...
        subprocess.run(["git", "-C", str(repo), "-c", "user.email=test@example.invalid", "-c", "user.name=test", "commit", "-qm", "v6"], check=True)
        head = subprocess.check_output(["git", "-C", str(repo), "rev-parse", "HEAD"], text=True).strip()
        archive = directory / "v6.skill.zip"
    ...
    def _envelope(self, installed: Path, receipt: dict[str, object], target: str = "010", input_ref: dict[str, object] | None = None) -> dict[str, object]:
        ...
        return {"schema_version": "task_input_envelope/v6", "adapter_version": "east-v5-runtime-adapter/v6", "issue_id": "EAS-70", "platform_parent_issue_id": "42f91f92-9453-4e7a-a38e-30d629ae07d6", "project_id": "3495da30-28d4-4cc6-b249-1e186ade6872", "run_id": "recovery-probe", "trace_id": "recovery-trace", "qa_id": "EAS70-RECOVERY", "attempt": 2, "target_agent_id": target, "target_agent_uuid": TARGETS[target][0], "root_binding_id": binding, "input_ref": input_ref, "input_receipt": None, "expected_output": {"artifact_type": route[0], "producer_id": route[1], "route_target": route[2]}, "recovery_of": {"old_run_id": "real-probe-010-110-120-v2", "old_issue_id": "0081927a-f1e7-42fe-a9e7-0a080857c082", "old_task_id": "c73f6821-0c29-4941-9214-12986df0d389", "decision_comment_id": "7397c95e-15b6-4b60-9d2d-5693d1e2659c", "run_id": "recovery-probe", "trace_id": "recovery-trace", "qa_id": "EAS70-RECOVERY"}, "supersedes": ["124a3e9d-fff7-4492-8c7b-9991d018795d", "a7d894d2-1c82-4bf2-a66a-5035404a6b4c"], "execution_bootstrap": {"bootstrap_version": "east-v5-runtime-bootstrap/v6", "candidate_base_sha": "9bdf86b1c5f369a8ee05bb94ba2e47aeb04414de", "candidate_head_sha": manifest["source_candidate_head"], "controller_sha256": manifest["source_hashes"]["scripts/controller.py"], "adapter_sha256": manifest["source_hashes"]["east_v5/runtime/controller_core.py"], "runtime_context": context, "skill_bundle": {"skill_name": "east-v5-runtime-bootstrap-v6", "skill_version": "v6", "skill_id": SKILL_ID, "skill_manifest_sha256": _sha(installed / "manifest.json"), "archive_sha256": receipt["archive_sha256"]}}}
    ...
    def _control(self, installed: Path, receipt: dict[str, object], target: str = "010", key_seed: bytes = b"v6-real-control-preflight") -> dict[str, object]:
        business = self._envelope(installed, receipt)
        key = hashlib.sha256(key_seed).hexdigest()
        return {"schema_version": "launcher_control_envelope/v1", "execution_mode": "launcher-control-preflight/v1", "issue_id": business["issue_id"], "platform_parent_issue_id": business["platform_parent_issue_id"], "project_id": business["project_id"], "run_id": "launcher-control-v6", "trace_id": "launcher-control-trace", "qa_id": "EAS70-LAUNCHER-CONTROL", "target_agent_id": target, "target_agent_uuid": TARGETS[target][0], "root_binding_id": business["root_binding_id"], "launch_idempotency_key": key, "execution_bootstrap": business["execution_bootstrap"], "callback": {"callback_agent_id": "1cdd93d3-b5fa-4dae-9b09-8320055c3072", "callback_issue_id": "42f91f92-9453-4e7a-a38e-30d629ae07d6", "callback_condition": "consume control task"}}
    ...
    def test_registry_conflict_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(first.returncode, 0, first.stderr)
            registry_path = runtime / "east-v5-runtime" / "issues" / "EAS-70" / "recovery-probe" / "2" / "artifact-registry.json"
    ...
    def test_launcher_failure_leaves_only_staged_nonconsumer_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            base = runtime / "east-v5-runtime" / "issues" / "EAS-70" / "recovery-probe" / "2"
