...
class RuntimeSkillV8Tests(unittest.TestCase):
    ...
    def _envelope(self, installed: Path, receipt: dict[str, object], target: str = "010", input_ref: dict[str, object] | None = None) -> dict[str, object]:
        ...
        context = {"resolver_version": "daemon_local_platform_data_resolver_v1", "workspace_id": "workspace", "project_id": "project", "daemon_id": "daemon", "runtime_root_resolver_version": "v5_runtime_root_env/v8", "runtime_root_fingerprint": "0" * 64}
        ...
        return {"schema_version": "task_input_envelope/v8", "adapter_version": "east-v5-runtime-adapter/v8", "issue_id": "EAS-70", "platform_parent_issue_id": "42f91f92-9453-4e7a-a38e-30d629ae07d6", "project_id": "3495da30-28d4-4cc6-b249-1e186ade6872", "run_id": "v8-root-transport-probe", "trace_id": "eas70-v8-root-transport", "qa_id": "EAS70-V8-ROOT", "attempt": 1, "target_agent_id": target, "target_agent_uuid": TARGETS[target][0], "root_binding_id": binding, "input_ref": input_ref, "input_receipt": None, "expected_output": {"artifact_type": route[0], "producer_id": route[1], "route_target": route[2]}, "recovery_of": {"old_run_id": "real-probe-010-110-120-v2", "old_issue_id": "0081927a-f1e7-42fe-a9e7-0a080857c082", "old_task_id": "c73f6821-0c29-4941-9214-12986df0d389", "decision_comment_id": "7397c95e-15b6-4b60-9d2d-5693d1e2659c", "run_id": "v8-root-transport-probe", "trace_id": "eas70-v8-root-transport", "qa_id": "EAS70-V8-ROOT"}, "supersedes": ["f2c2d787-a6dc-4460-bcdc-1f71a6e27292", "72965778-39aa-441d-a1c3-f96e96ebfe32"], "execution_bootstrap": {"bootstrap_version": "east-v5-runtime-bootstrap/v8", "candidate_base_sha": "53480044c9651aa895795cada5be7a0bf3973caf", "candidate_head_sha": manifest["source_candidate_head"], "controller_sha256": manifest["source_hashes"]["scripts/controller.py"], "adapter_sha256": manifest["source_hashes"]["east_v5/runtime/controller_core.py"], "runtime_context": context, "skill_bundle": {"skill_name": "east-v5-runtime-bootstrap-v8", "skill_version": "v8", "skill_id": SKILL_ID, "skill_manifest_sha256": _sha(installed / "manifest.json"), "archive_sha256": receipt["archive_sha256"]}}}
    ...
    def _call(self, installed: Path, command: str, envelope: dict[str, object], claim: dict[str, object], directory: Path, *extra: str) -> subprocess.CompletedProcess[str]:
        values = list(extra)
        runtime_root = directory / "governed-runtime-root"
        if "--runtime-root" in values:
            index = values.index("--runtime-root")
            runtime_root = Path(values[index + 1])
            del values[index:index + 2]
        runtime_root.mkdir(parents=True, exist_ok=True)
        stat = runtime_root.resolve().stat()
        context = envelope["execution_bootstrap"]["runtime_context"]
        context["runtime_root_fingerprint"] = hashlib.sha256(json.dumps({"resolver_version": "v5_runtime_root_env/v8", "workspace_id": context["workspace_id"], "project_id": context["project_id"], "daemon_id": context["daemon_id"], "device": stat.st_dev, "inode": stat.st_ino}, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
        envelope["root_binding_id"] = hashlib.sha256(json.dumps(context, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
        ...
        env = {"PATH": os.environ["PATH"], "V5_RUNTIME_ROOT": str(runtime_root.resolve()), "V5_RUNTIME_DAEMON_ID": "daemon"}
        return subprocess.run([sys.executable, "-I", str(installed / "scripts" / "controller.py"), command, "--envelope-file", str(envelope_path), "--claim-file", str(claim_path), *values], cwd=directory, env=env, text=True, capture_output=True, check=False)
    ...
    def test_root_transport_preflight_uses_one_env_root_from_three_workdirs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp)
            _archive, receipt, installed = self._archive(directory)
            shared = directory / "governed-root"
            results = []
            for target in ("010", "110", "120"):
                workdir = directory / f"{target}-workdir"; workdir.mkdir()
                reference = None if target == "010" else {"artifact_id": "root-transport-canary", "version": 1, "content_hash": "a" * 64}
                envelope = self._envelope(installed, receipt, target, reference)
                if reference is not None:
                    inbound = {"output_ref": reference, "route_target": target, "run_id": envelope["run_id"], "trace_id": envelope["trace_id"], "qa_id": envelope["qa_id"], "attempt": envelope["attempt"]}
                    inbound["content_hash"] = hashlib.sha256(json.dumps(inbound, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
                    envelope["input_receipt"] = inbound
                result = self._call(installed, "root-transport-preflight", envelope, self._claim(target, receipt), workdir, "--runtime-root", str(shared))
                self.assertEqual(result.returncode, 0, result.stderr)
                results.append(json.loads(result.stdout))
            self.assertEqual({value["runtime_root_fingerprint"] for value in results}, {results[0]["runtime_root_fingerprint"]})
            self.assertTrue(all(value["business_artifact_count"] == value["business_receipt_count"] == value["downstream_task_count"] == 0 for value in results))
    ...
    def test_registry_conflict_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(first.returncode, 0, first.stderr)
            registry_path = runtime / "east-v5-runtime" / "issues" / "EAS-70" / "v8-root-transport-probe" / "1" / "artifact-registry.json"
    ...
    def test_rejects_claim_instruction_hash_and_supersedes_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            envelope = self._envelope(installed, receipt); envelope["attempt"] = 3
    ...
    def test_launcher_failure_leaves_only_staged_nonconsumer_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            base = runtime / "east-v5-runtime" / "issues" / "EAS-70" / "v8-root-transport-probe" / "1"
