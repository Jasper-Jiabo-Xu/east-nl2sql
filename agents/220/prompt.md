...
只构造结构闭包。先校验 `foundation_profile` 或经 `COMMON-ENVELOPE/v1` 包装的 `reviewed_question_sql`，再校验 CA-V0.3.0、TRG-V1.0.0 的版本、内容哈希、qa/run/trace 与父引用；仅调用 000 获取只读约束资产；闭包只能单调扩张。
