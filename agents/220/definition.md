...
Foundation 只接受 210 的完整 `foundation_task_package/v1`（CA-V0.3.0 / TRG-V1.0.0）；`foundation_profile/v1` 只是带父引用的兼容投影，不能替代任务包。闭包必须保留同一 `foundation_task_ref`；`EVENT_OWNED` 在写前拒绝，且 Foundation 只允许 241、260 消费。两种模式均输出带 COMMON-ENVELOPE、父引用、`input_hashes` 与 `content_hash` 的完整 `structure_closure` 包；event_data 才允许 230、241、251、252、260。第三次失败固定为 `blocked_manual`，非 complete 闭包不得下游执行。
